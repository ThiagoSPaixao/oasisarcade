...
export function AnalogPad({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      <div
        ...
        className="border-accent/35 bg-surface/40 relative h-32 w-32 shrink-0 touch-none rounded-full border shadow-[0_0_36px_-14px_var(--neon-cyan)] sm:h-36 sm:w-36"
      >
        <span
          style={{ transform: `translate(calc(-50% + ${knob.x}px), calc(-50% + ${knob.y}px))` }}
          className={cn(
            "border-accent/60 bg-surface-2/80 absolute top-1/2 left-1/2 h-14 w-14 rounded-full border backdrop-blur transition-transform",
